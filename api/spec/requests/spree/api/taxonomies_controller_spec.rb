  ...
  describe Api::TaxonomiesController, type: :request do
    ...
    context "as a normal user" do
      it "gets all taxonomies" do
        get spree.api_taxonomies_path, headers: user.create_new_auth_token
      ...
      it 'can control the page size through a parameter' do
        create(:taxonomy)
        get spree.api_taxonomies_path, headers: user.create_new_auth_token, params: { per_page: 1 }
      ...
      it 'can query the results through a paramter' do
        ...
        get spree.api_taxonomies_path, headers: user.create_new_auth_token, params: { q: { name_cont: 'style' } }
        expect(json_response['count']).to eq(1)
        expect(json_response['taxonomies'].first['name']).to eq expected_result.name
      end

      it "gets a single taxonomy" do
        get spree.api_taxonomy_path(taxonomy.id), headers: user.create_new_auth_token
      ...
      it "gets a single taxonomy with set=nested" do
        get spree.api_taxonomy_path(taxonomy.id), headers: user.create_new_auth_token, params: { set: 'nested' }
      ...
      it "gets the jstree-friendly version of a taxonomy" do
        ...
        get spree.jstree_api_taxonomy_path(taxonomy.id), headers: user.create_new_auth_token
      ...
      it "can learn how to create a new taxonomy" do
        get spree.new_api_taxonomy_path(taxonomy.id), headers: user.create_new_auth_token
      ...
      it "cannot create a new taxonomy if not an admin" do
        post spree.api_taxonomies_path, headers: user.create_new_auth_token, params: { taxonomy: { name: "Location" } }
        assert_unauthorized!
      end

      it "cannot update a taxonomy" do
        put spree.api_taxonomy_path(taxonomy.id), headers: user.create_new_auth_token, params: { taxonomy: { name: "I hacked your store!" } }
        assert_unauthorized!
      end

      it "cannot delete a taxonomy" do
        delete spree.api_taxonomy_path(taxonomy.id), headers: user.create_new_auth_token
        assert_unauthorized!
      end
    end
    ...
    context "as an admin" do
      it "can create" do
        post spree.api_taxonomies_path,
          headers: admin_user.create_new_auth_token,
          params: { taxonomy: { name: "Colors" } }
