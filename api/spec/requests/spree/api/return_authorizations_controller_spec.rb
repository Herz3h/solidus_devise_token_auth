...
module Spree
  describe Api::ReturnAuthorizationsController, type: :request do
    ...
    let!(:other_order) { create(:shipped_order, user: other_user) }
    ...
    shared_examples_for 'a return authorization creator' do
      it "can create a new return authorization" do
        ...
        post spree.api_order_return_authorizations_path(order),
          ...
          params: { order_id: order.number, return_authorization: rma_params }
    ...
    context "as the order owner" do
      it "cannot see any return authorizations" do
        get spree.api_order_return_authorizations_path(order), headers: user.create_new_auth_token
        assert_unauthorized!
      end

      it "cannot see a single return authorization" do
        get spree.api_order_return_authorization_path(order, 1), headers: user.create_new_auth_token
        assert_unauthorized!
      end

      it "cannot learn how to create a new return authorization" do
        get spree.new_api_order_return_authorization_path(order), headers: user.create_new_auth_token
        assert_unauthorized!
      end
      ...
      it "cannot update a return authorization" do
        put spree.api_order_return_authorization_path(order, 0), headers: user.create_new_auth_token
        assert_not_found!
      end

      it "cannot delete a return authorization" do
        delete spree.api_order_return_authorization_path(order, 0), headers: user.create_new_auth_token
        assert_not_found!
      end
    end
    ...
    context "as another non-admin user that's not the order's owner" do
      it "cannot create a new return authorization" do
        post spree.api_order_return_authorizations_path(order), headers: other_user.create_new_auth_token
        assert_unauthorized!
      end
    end
    ...
    context "as an admin" do
      it "can show return authorization" do
        ...
        get spree.api_order_return_authorization_path(order, return_authorization.id), headers: admin_user.create_new_auth_token
      ...
      it 'can control the page size through a parameter' do
        ...
        get spree.api_order_return_authorizations_path(order),
          headers: admin_user.create_new_auth_token,
          params: { order_id: order.number, per_page: 1 }
      ...
      it "can learn how to create a new return authorization" do
        get spree.new_api_order_return_authorization_path(order), headers: admin_user.create_new_auth_token
      ...
      it "can update a return authorization on the order" do
        ...
        put spree.api_order_return_authorization_path(order, return_authorization.id),
          headers: admin_user.create_new_auth_token,
          params: { return_authorization: { memo: "ABC" } }
      ...
      it "can cancel a return authorization on the order" do
        ...
        put spree.cancel_api_order_return_authorization_path(order, return_authorization.id), headers: admin_user.create_new_auth_token
        expect(response.status).to eq(200)
      ...
      it "can delete a return authorization on the order" do
        FactoryBot.create(:return_authorization, order: order)
        return_authorization = order.return_authorizations.first
        delete spree.api_order_return_authorization_path(order, return_authorization.id), headers: admin_user.create_new_auth_token
    ...
    context "as just another user" do
      it "cannot add a return authorization to the order" do
        post spree.api_order_return_authorizations_path(order),
          ...
          params: { return_autorization: { order_id: order.number, memo: "Defective" } }
      ...
      it "cannot delete a return authorization on the order" do
        FactoryBot.create(:return_authorization, order: order)
        return_authorization = order.return_authorizations.first
        delete spree.api_order_return_authorization_path(order, return_authorization.id), headers: other_user.create_new_auth_token
        assert_unauthorized!
